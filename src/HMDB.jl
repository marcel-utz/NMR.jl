...
function matchPeaks(p::NMR.PeakStruct,d,i;tol=0.001)
    ...
    for k=1:length(d)
        ...
        elseif start(r)==1 && isempty(r)
            closest=p.positions[1]
            cInt=p.intensities[1]

        elseif !isempty(r)
            closest=p.positions[start(r)]
            cInt=p.intensities[start(r)]
        else
            closest= abs(p.positions[start(r)]-d[k])<abs(p.positions[start(r)-1]-d[k])?p.positions[start(r)] : p.positions[start(r)-1]
            cInt= abs(p.positions[start(r)]-d[k])<abs(p.positions[start(r)-1]-d[k])?p.intensities[start(r)] : p.intensities[start(r)-1]
    ...
    return (score/maximum(i),score/sum(i),Pk/q2, (p2-Pk*Pk/q2)/q2/(length(d)-1))
end

function matchPeaks(p::NMR.PeakStruct,b::HMDBpeaks;opts...)
    return matchPeaks(p,b.pks,b.ints;opts...)
end
