...
function process_fid(n,dataStore,dataLocation)
        ...
        spect=NMR.BaseLineCorrect(spect,order=23)
        return spect
end
...
function generate_protocol(fname)
    ...
    infile=open(d[protof],"r")
    outfile=open("protocol-raw.csv","w")
    ...
    try 
        tlograw=CSV.read(d[boxf], DataFrame,types=[String,Float64,Float64,Float64,Float64,Float64,Float64,String])
    catch
    end
    ...
    protocol=CSV.read("protocol-raw.csv",DataFrame)
    ...
    CSV.write("protocol-final.csv",protocol_final)
