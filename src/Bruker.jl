...
function readFID(f::IOStream; format=Float64)
  data = Array{format,1}([])
  while !eof(f)
...
"""
`Bruker.readFID(s::String; format=Float64)` reads a Bruker fid or ser file and returns
...
`format=Int32`.
"""
function readFID(s::String;opts...)
    f=open(s,"r")
    fid=readFID(f;opts...)
    close(f)
    return fid
